...
if settings.DATASET == "MIRFlickr":
    ...
    label_set = scio.loadmat(settings.LABEL_DIR)
    label_set = np.array(label_set['LAll'], dtype=np.float)   # 按 标号id升序的标签向量
    ...
    indexTest = test_index              # 测试和数据库互斥 他俩加起来是20015个
    indexDatabase = database_index      # 数据库包含训练集
    indexTrain = train_index           # 用做训练的标号id
