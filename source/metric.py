...
def compress(train_loader, test_loader, model_I, model_T, train_dataset, test_dataset):
    re_BI = list([])
    re_BT = list([])
    re_L = list([])   #  定义保存 数据库 图像 文本 标签 的二进制码
    for _, (data_I, data_T, _, _) in enumerate(train_loader):
        var_data_I = Variable(data_I.cuda())
        _,_,code_I = model_I(var_data_I)
        code_I = torch.sign(code_I)
        re_BI.extend(code_I.cpu().data.numpy())  # 数据库 图像的二进制码
        
        var_data_T = Variable(torch.FloatTensor(data_T.numpy()).cuda())
        _,_,code_T = model_T(var_data_T)
        code_T = torch.sign(code_T)
        re_BT.extend(code_T.cpu().data.numpy())   # 数据库 文本的二进制码

    qu_BI = list([])
    qu_BT = list([])
    qu_L = list([])   #  定义保存 测试集 图像 文本 标签 的二进制码
    for _, (data_I, data_T, _, _) in enumerate(test_loader):
        var_data_I = Variable(data_I.cuda())
        _,_,code_I = model_I(var_data_I)
        code_I = torch.sign(code_I)
        qu_BI.extend(code_I.cpu().data.numpy())  # 测试集 图像的二进制码
        
        var_data_T = Variable(torch.FloatTensor(data_T.numpy()).cuda())
        _,_,code_T = model_T(var_data_T)
        code_T = torch.sign(code_T)
        qu_BT.extend(code_T.cpu().data.numpy())   # 测试集 文本的二进制码
    ...
    # print("save in", settings.save_path)
    # os.chdir(settings.save_path)
    # np.save(settings.save_path + str(settings.CODE_LEN) + "-" + "database_imgb.npy", re_BI)
    return re_BI, re_BT, re_L, qu_BI, qu_BT, qu_L
